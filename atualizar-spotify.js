const fs = require('fs');

// URL da imagem dinâmica do seu Spotify
const spotifyBadge = `<img src="https://vercel.app{Date.now()}" alt="Spotify Recently Played" />`;

// Todo o conteúdo do seu README estruturado em texto puro
const readmeCompleto = `![Banner](https://githubusercontent.com)

<h1 align="center">Pedro Henrique &nbsp;·&nbsp; Desenvolvedor Web & Mobile</h1>

<p align="center">
  <a href="https://linkedin.com" target="_blank">
    <img src="https://shields.io" height="22" alt="LinkedIn" />
  </a>
  &nbsp;
  <a href="https://instagram.com" target="_blank">
    <img src="https://shields.io" height="22" alt="Instagram" />
  </a>
  &nbsp;
  <a href="SEU_DISCORD_AQUI" target="_blank">
    <img src="https://shields.io" height="22" alt="Discord" />
  </a>
  &nbsp;
  <a href="mailto:SEU_EMAIL_AQUI">
    <img src="https://shields.io" height="22" alt="Gmail" />
  </a>
</p>

---

## Sobre mim

<img align="left" src="https://githubusercontent.com" width="180" style="margin-right: 20px; border-radius: 12px;" />

Desenvolvedor Web & Mobile com background em suporte técnico, experiência que me deu uma visão analítica voltada à solução de problemas reais. Aplico esse olhar no desenvolvimento de aplicações modernas com foco em boas práticas, arquitetura limpa e código sustentável.

Meu objetivo é entregar soluções que unem **performance**, **usabilidade** e **qualidade técnica**, tanto em projetos pessoais quanto nas demandas de clientes.

<br />
<br />

\`\`\`text
🎓  ADS · Técnico em Desenvolvimento Web & Mobile
💼  Estagiário de Suporte → migrando para desenvolvimento full-time
📚  Estudando: Arquitetura Limpa · Cybersecurity · Automação com n8n
🏎️  Entusiasta de automobilismo (F1), games e esportes de combate
\`\`\`

<br clear="left" />

---

## Stack

**Linguagens**

![JavaScript](https://shields.io)
![TypeScript](https://shields.io)
![Python](https://shields.io)
![HTML5](https://shields.io)
![CSS3](https://shields.io)

**Frameworks & Bibliotecas**

![React](https://shields.io)
![Next.js](https://shields.io)
![Node.js](https://shields.io)
![Express](https://shields.io)
![Django](https://shields.io)

**Banco de Dados**

![PostgreSQL](https://shields.io)
![MySQL](https://shields.io)
![SQL Server](https://shields.io)

**Ferramentas & Ambiente**

![Git](https://shields.io)
![GitHub](https://shields.io)
![VS Code](https://shields.io)
![Figma](https://shields.io)
![n8n](https://shields.io)
![Supabase](https://shields.io)
![Android Studio](https://shields.io)

---

## GitHub Stats

<div align="center">
  <img src="https://demolab.com" height="155" alt="GitHub Streak" />
</div>

---

## Vamos conversar?

Se você tem um projeto em mente, quer automatizar processos ou precisa de um desenvolvedor comprometido com qualidade, pode me chamar. Estou sempre aberto a novas oportunidades e colaborações! 🚀

---

<details>
  <summary>🎧 Ouvindo agora</summary>
  <br>
  <div align="center">
    ${spotifyBadge}
  </div>
</details>
`;

try {
  // Escreve e substitui o arquivo inteiro do README.md de uma vez só
  fs.writeFileSync('README.md', readmeCompleto.trim(), 'utf8');
  console.log('README.md completo gerado com sucesso!');
} catch (error) {
  console.error('Erro ao gerar o README:', error);
  process.exit(1);
}
